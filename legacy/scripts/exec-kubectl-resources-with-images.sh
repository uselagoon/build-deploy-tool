...
if [[ $(helm show values /kubectl-build-deploy/helmcharts/${SERVICE_TYPE} | grep "images:") ]]; then
  ...
  helm template ${SERVICE_NAME} /kubectl-build-deploy/helmcharts/${SERVICE_TYPE} -f /kubectl-build-deploy/values.yaml -f /kubectl-build-deploy/${SERVICE_NAME}-values.yaml -f /kubectl-build-deploy/extravolumes-values.yaml --set service_name="${SERVICE_NAME}" "${HELM_SET_VALUES[@]}" "${HELM_ARGUMENTS[@]}"  > $YAML_FOLDER/${SERVICE_NAME}.yaml
...
# check if we need a single image to inject
  ...
  helm template ${SERVICE_NAME} /kubectl-build-deploy/helmcharts/${SERVICE_TYPE} -f /kubectl-build-deploy/values.yaml -f /kubectl-build-deploy/${SERVICE_NAME}-values.yaml  -f /kubectl-build-deploy/extravolumes-values.yaml --set image="${SERVICE_NAME_IMAGE_HASH}" "${HELM_SET_VALUES[@]}" "${HELM_ARGUMENTS[@]}"  > $YAML_FOLDER/${SERVICE_NAME}.yaml
elif [ -d /kubectl-build-deploy/helmcharts/${SERVICE_TYPE} ]; then
  cat /kubectl-build-deploy/${SERVICE_NAME}-values.yaml
  helm template ${SERVICE_NAME} /kubectl-build-deploy/helmcharts/${SERVICE_TYPE} -f /kubectl-build-deploy/values.yaml -f /kubectl-build-deploy/${SERVICE_NAME}-values.yaml  -f /kubectl-build-deploy/extravolumes-values.yaml "${HELM_SET_VALUES[@]}" "${HELM_ARGUMENTS[@]}"  > $YAML_FOLDER/${SERVICE_NAME}.yaml
fi
