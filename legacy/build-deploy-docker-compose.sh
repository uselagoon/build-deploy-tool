...
DBAAS_VARIABLES="[]"
for MARIADB_CONSUMER_NUMBER in "${!MARIADB_DBAAS_CONSUMER_SPECS[@]}"
do
  SERVICE_NAME=$(echo ${MARIADB_DBAAS_CONSUMER_SPECS["$MARIADB_CONSUMER_NUMBER"]} | jq -Rr '@base64d | fromjson | .metadata.name')
  SERVICE_NAME_UPPERCASE=$(echo "$SERVICE_NAME" | tr '[:lower:]' '[:upper:]' | tr '-' '_')
  DB_HOST=$(echo ${MARIADB_DBAAS_CONSUMER_SPECS["$MARIADB_CONSUMER_NUMBER"]} | jq -Rr '@base64d | fromjson | .spec.consumer.services.primary')
  DB_USER=$(echo ${MARIADB_DBAAS_CONSUMER_SPECS["$MARIADB_CONSUMER_NUMBER"]} | jq -Rr '@base64d | fromjson | .spec.consumer.username')
  DB_PASSWORD=$(echo ${MARIADB_DBAAS_CONSUMER_SPECS["$MARIADB_CONSUMER_NUMBER"]} | jq -Rr '@base64d | fromjson | .spec.consumer.password')
  DB_NAME=$(echo ${MARIADB_DBAAS_CONSUMER_SPECS["$MARIADB_CONSUMER_NUMBER"]} | jq -Rr '@base64d | fromjson | .spec.consumer.database')
  DB_PORT=$(echo ${MARIADB_DBAAS_CONSUMER_SPECS["$MARIADB_CONSUMER_NUMBER"]} | jq -Rr '@base64d | fromjson | .spec.provider.port')
  ...
  if DB_READREPLICA_HOSTS=$(echo ${MARIADB_DBAAS_CONSUMER_SPECS["$MARIADB_CONSUMER_NUMBER"]} | jq -Rr '@base64d | fromjson | .spec.consumer.services.replicas | .[]' 2>/dev/null); then
...
for POSTGRES_CONSUMER_NUMBER in "${!POSTGRES_DBAAS_CONSUMER_SPECS[@]}"
do
  SERVICE_NAME=$(echo ${POSTGRES_DBAAS_CONSUMER_SPECS["$POSTGRES_CONSUMER_NUMBER"]} | jq -Rr '@base64d | fromjson | .metadata.name')
  SERVICE_NAME_UPPERCASE=$(echo "$SERVICE_NAME" | tr '[:lower:]' '[:upper:]' | tr '-' '_')
  DB_HOST=$(echo ${POSTGRES_DBAAS_CONSUMER_SPECS["$POSTGRES_CONSUMER_NUMBER"]} | jq -Rr '@base64d | fromjson | .spec.consumer.services.primary')
  DB_USER=$(echo ${POSTGRES_DBAAS_CONSUMER_SPECS["$POSTGRES_CONSUMER_NUMBER"]} | jq -Rr '@base64d | fromjson | .spec.consumer.username')
  DB_PASSWORD=$(echo ${POSTGRES_DBAAS_CONSUMER_SPECS["$POSTGRES_CONSUMER_NUMBER"]} | jq -Rr '@base64d | fromjson | .spec.consumer.password')
  DB_NAME=$(echo ${POSTGRES_DBAAS_CONSUMER_SPECS["$POSTGRES_CONSUMER_NUMBER"]} | jq -Rr '@base64d | fromjson | .spec.consumer.database')
  DB_PORT=$(echo ${POSTGRES_DBAAS_CONSUMER_SPECS["$POSTGRES_CONSUMER_NUMBER"]} | jq -Rr '@base64d | fromjson | .spec.provider.port')
  DB_CONSUMER='{"'${SERVICE_NAME_UPPERCASE}'_HOST":"'${DB_HOST}'", "'${SERVICE_NAME_UPPERCASE}'_USERNAME":"'${DB_USER}'","'${SERVICE_NAME_UPPERCASE}'_PASSWORD":"'${DB_PASSWORD}'","'${SERVICE_NAME_UPPERCASE}'_DATABASE":"'${DB_NAME}'","'${SERVICE_NAME_UPPERCASE}'_PORT":"'${DB_PORT}'"}'
  if DB_READREPLICA_HOSTS=$(echo ${POSTGRES_DBAAS_CONSUMER_SPECS["$POSTGRES_CONSUMER_NUMBER"]} | jq -Rr '@base64d | fromjson | .spec.consumer.services.replicas | .[]' 2>/dev/null); then
    if [ "$DB_READREPLICA_HOSTS" != "null" ]; then
      DB_READREPLICA_HOSTS=$(echo "$DB_READREPLICA_HOSTS" | sed 's/^\|$//g' | paste -sd, -)
      DB_CONSUMER=$(echo "${DB_CONSUMER}" | jq '. + {"'${SERVICE_NAME_UPPERCASE}'_READREPLICA_HOSTS":"'${DB_READREPLICA_HOSTS}'"}')
    fi
...
for MONGODB_CONSUMER_NUMBER in "${!MONGODB_DBAAS_CONSUMER_SPECS[@]}"
do
  SERVICE_NAME=$(echo ${MONGODB_DBAAS_CONSUMER_SPECS["$MONGODB_CONSUMER_NUMBER"]} | jq -Rr '@base64d | fromjson | .metadata.name')
  SERVICE_NAME_UPPERCASE=$(echo "$SERVICE_NAME" | tr '[:lower:]' '[:upper:]' | tr '-' '_')
  DB_HOST=$(echo ${MONGODB_DBAAS_CONSUMER_SPECS["$MONGODB_CONSUMER_NUMBER"]} | jq -Rr '@base64d | fromjson | .spec.consumer.services.primary')
  DB_USER=$(echo ${MONGODB_DBAAS_CONSUMER_SPECS["$MONGODB_CONSUMER_NUMBER"]} | jq -Rr '@base64d | fromjson | .spec.consumer.username')
  DB_PASSWORD=$(echo ${MONGODB_DBAAS_CONSUMER_SPECS["$MONGODB_CONSUMER_NUMBER"]} | jq -Rr '@base64d | fromjson | .spec.consumer.password')
  DB_NAME=$(echo ${MONGODB_DBAAS_CONSUMER_SPECS["$MONGODB_CONSUMER_NUMBER"]} | jq -Rr '@base64d | fromjson | .spec.consumer.database')
  DB_PORT=$(echo ${MONGODB_DBAAS_CONSUMER_SPECS["$MONGODB_CONSUMER_NUMBER"]} | jq -Rr '@base64d | fromjson | .spec.provider.port')
  DB_AUTHSOURCE=$(echo ${MONGODB_DBAAS_CONSUMER_SPECS["$MONGODB_CONSUMER_NUMBER"]} | jq -Rr '@base64d | fromjson | .spec.provider.auth.source')
  DB_AUTHMECHANISM=$(echo ${MONGODB_DBAAS_CONSUMER_SPECS["$MONGODB_CONSUMER_NUMBER"]} | jq -Rr '@base64d | fromjson | .spec.provider.auth.mechanism')
  DB_AUTHTLS=$(echo ${MONGODB_DBAAS_CONSUMER_SPECS["$MONGODB_CONSUMER_NUMBER"]} | jq -Rr '@base64d | fromjson | .spec.provider.auth.tls')
