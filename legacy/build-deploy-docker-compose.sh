...
if [ ! -z "$EXISTING_LAGOONENV_VARS" ]; then
  ...
  # get all the unique keys from the API and current secret
  # and remove anything that isn't in the API anymore from the secret
  VARS_TO_REMOVE1=$(comm -23 <(echo $CURRENT_LAGOONENV_VARS | tr ' ' '\n' | sort) <(echo $API_ENVIRONMENT_VARS | tr ' ' '\n' | sort))
...
CURRENT_LAGOONPLATFORMENV_VARS=$(kubectl -n ${NAMESPACE} get secret lagoon-platform-env -o json  2> /dev/null | jq -r '.data | keys[]')
CURRENT_LAGOONENV_VARS=$(kubectl -n ${NAMESPACE} get secret lagoon-env -o json  2> /dev/null | jq -r '.data | keys[]')
...
# write the secret to the values file so when we `exec-kubectl-resources-with-images.sh` the deployments will get the value of the secret
# which will cause a change in the deployment and trigger a rollout if only the secret has changed
