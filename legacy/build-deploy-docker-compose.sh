...
FASTLY_API_SECRETS=()
if [ "$(cat .lagoon.yml | yq -o json | jq -r '.fastly."api-secrets"['$FASTLY_API_SECRETS_COUNTER']')" != "null" ]; then
  while [ "$(cat .lagoon.yml | yq -o json | jq -r '.fastly."api-secrets"['$FASTLY_API_SECRETS_COUNTER']')" != "null" ]; do
    FASTLY_API_SECRET_NAME=$FASTLY_API_SECRET_PREFIX$(cat .lagoon.yml | yq -o json | jq -r '.fastly."api-secrets"['$FASTLY_API_SECRETS_COUNTER'].name' 2> /dev/null)
    ...
    FASTLY_API_TOKEN_VALUE=$(cat .lagoon.yml | yq -o json | jq -r '.fastly."api-secrets"['$FASTLY_API_SECRETS_COUNTER'].apiTokenVariableName // false')
    ...
    FASTLY_API_PLATFORMTLS_CONFIGURATION=$(cat .lagoon.yml | yq -o json | jq -r '.fastly."api-secrets"['$FASTLY_API_SECRETS_COUNTER'].platformTLSConfiguration // ""')
