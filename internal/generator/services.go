	...
	if lagoonType == "" {
		...
		if lagoonOverrideName != "" {
			...
			for _, service := range buildValues.Services {
				...
				// for this service  as it is probably a dual container service (eg, nginx-php/nginx-php-persistent)
		...
		if lagoonType == "external" {
			...
			// check if the service has a workloadresource request
			serviceResourceWorkload = lagoon.CheckDockerComposeLagoonLabel(composeServiceValues.Labels, "lagoon.workloadresource")
			// check for an api override on the resource workload or if one isn't found in the docker-compose file
			getResourceWorkloadOverride(buildValues, &serviceResourceWorkload, lagoonOverrideName)
			for _, service := range buildValues.Services {
				// if there is an existing service with this same override name, then disable resource workloads for this service as it is probably a
				// dual container service (eg, nginx-php/nginx-php-persistent)
				if service.OverrideName == lagoonOverrideName {
					serviceResourceWorkload = ""
				}
			}
			if serviceResourceWorkload != "" {
				// check if the requested resource workload actually exists
				resWork, ok := buildValues.ResourceWorkloads[serviceResourceWorkload]
				if !ok {
					return nil, fmt.Errorf(
						"the requested scaling parameter group %s for service %s is not valid",
						serviceResourceWorkload, composeService,
					)
				}
				if resWork.ServiceType != lagoonType {
					return nil, fmt.Errorf(
						"the requested scaling parameter group %s for service %s is not valid for this service type %s",
						serviceResourceWorkload, composeService, lagoonType,
					)
				}
			}
		...
		cService := &ServiceValues{
			...
			ResourceWorkload:                       serviceResourceWorkload,
		}
...
// getResourceWorkloadOverride will check the api variables for a resource override
func getResourceWorkloadOverride(
	buildValues *BuildValues,
	workloadOverrides *string,
	lagoonOverrideName string,
) {
	if buildValues.ResourceWorkloadOverrides != "" {
		workloadOverridesSplit := strings.Split(buildValues.ResourceWorkloadOverrides, ",")
		for _, sType := range workloadOverridesSplit {
			sTypeSplit := strings.Split(sType, ":")
			if sTypeSplit[0] == lagoonOverrideName {
				*workloadOverrides = sTypeSplit[1]
			}
		}
	}
}
