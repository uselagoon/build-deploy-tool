...
func composeToServiceValues(
	...
	if lagoonType == "" {
		...
		serviceResourceWorkload := lagoon.CheckServiceLagoonLabel(composeServiceValues.Labels, "lagoon.workloadresource")
		// check for an api override on the resource workload or if one isn't found in the docker-compose file
		getResourceWorkloadOverride(buildValues, &serviceResourceWorkload, lagoonOverrideName, lagoonType, debug)
		for _, service := range buildValues.Services {
