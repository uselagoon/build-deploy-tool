...
func composeToServiceValues(
	...
	if lagoonType == "" {
		...
		lagoonType = servicetypes.ConvertOldServiceType(lagoonType)
		...
		// finally check that the service type is a supported service type, otherwise return an error saying the type is not valid
		if !servicetypes.IsSupportedType(lagoonType) {
			return nil, fmt.Errorf(
				"lagoon.type '%s' is not a valid service type. See the Lagoon documentation for supported service types",
				lagoonType,
			)
		}
		...
		if lagoonType == "external" {
			...
			if servicetypes.IsSupportedDBType(lagoonType) {
			...
			if !servicetypes.IsAutogeneratedSupported(lagoonType) {
				autogenEnabled = false
				autogenTLSAcmeEnabled = false
			}

			// check if this service is one that supports backups
			if servicetypes.IsTypeWithBackups(lagoonType) {
				backupsEnabled = true
		...
		if !servicetypes.IsIgnoredImageType(lagoonType) {
