	...
	for _, service := range lComposeOrder {
		for _, composeServiceValues := range lCompose.Services {
			if service.Name == composeServiceValues.Name {
				cService, err := composeToServiceValues(buildValues, lagoonEnvVars, lYAML, composeServiceValues.Name, composeServiceValues, debug)
				if err != nil {
					return err
				}
...
func composeToServiceValues(
	...
	if lagoonType == "" {
		...
		// spotReplicaTypes := CheckAdminFeatureFlag("SPOT_REPLICAS_PRODUCTION", lagoonEnvVars, debug) // doesn't exist yet

		productionSpot := CheckFeatureFlag("SPOT_INSTANCE_PRODUCTION", lagoonEnvVars, debug)
		developmentSpot := CheckFeatureFlag("SPOT_INSTANCE_DEVELOPMENT", lagoonEnvVars, debug)
		if productionSpot == "enabled" && buildValues.EnvironmentType == "production" {
			spotTypes = CheckFeatureFlag("SPOT_INSTANCE_PRODUCTION_TYPES", lagoonEnvVars, debug)
			cronjobSpotTypes = CheckFeatureFlag("SPOT_INSTANCE_PRODUCTION_CRONJOB_TYPES", lagoonEnvVars, debug)
		}
		if developmentSpot == "enabled" && buildValues.EnvironmentType == "development" {
			spotTypes = CheckFeatureFlag("SPOT_INSTANCE_DEVELOPMENT_TYPES", lagoonEnvVars, debug)
			cronjobSpotTypes = CheckFeatureFlag("SPOT_INSTANCE_DEVELOPMENT_CRONJOB_TYPES", lagoonEnvVars, debug)
		}
