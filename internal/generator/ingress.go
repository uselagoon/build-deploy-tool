...
func generateRoutes(lagoonEnvVars []lagoon.EnvironmentVariable,
	...
	if activeEnv || standbyEnv {
		...
		*activeStanbyRoutes = generateActiveStandbyRoutes(activeEnv, standbyEnv, lagoonEnvVars, lYAML, lagoonValues)
		// get the first route from the list of routes, replace the previous one if necessary
...
func generateIngress(values BuildValues, lYAML lagoon.YAML, envVars []lagoon.EnvironmentVariable, mainRoutes *lagoon.RoutesV2, debug bool) error {
		...
		for serviceName, service := range lagoonValues.Services {
			...
			if service.AutogeneratedRoutesEnabled {
				if helpers.Contains(supportedAutogeneratedTypes, service.Type) {
					...
					domain, shortDomain := autogeneratedDomainFromPattern(lagoonRouterPattern.Value, serviceOverrideName, lagoonValues.Project, lagoonValues.Environment)
					...
					err := lagoon.GenerateFastlyConfiguration(fastlyConfig, lagoonValues.FastlyCacheNoCache, lagoonValues.Fastly.ServiceID, domain, lagoonValues.FastlyAPISecretPrefix, envVars)
					if err != nil {
						return err
					}
					insecure := "Allow"
			...
			if lagoonYAML.ProductionRoutes.Active != nil {
				if lagoonYAML.ProductionRoutes.Active.Routes != nil {
					for _, routeMap := range lagoonYAML.ProductionRoutes.Active.Routes {
						lagoon.GenerateRoutesV2(activeStanbyRoutes, routeMap, envVars, lagoonValues.FastlyAPISecretPrefix, true)
					}
				}
			}
		}
...
func generateAndMerge(
	...
	for _, routeMap := range lagoonYAML.Environments[lagoonValues.Branch].Routes {
		lagoon.GenerateRoutesV2(n, routeMap, envVars, lagoonValues.FastlyAPISecretPrefix, false)
	}
	// merge routes from the API on top of the routes from the `.lagoon.yml`
	mainRoutes := lagoon.MergeRoutesV2(*n, api, envVars, lagoonValues.FastlyAPISecretPrefix)
