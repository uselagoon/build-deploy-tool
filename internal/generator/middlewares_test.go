...
func Test_serverSnippetClearHeader(t *testing.T) {
	tests := []struct {
		name string // description of this test case
		// Named input parameters for target function.
		snippet string
...
set_real_ip_from 1.2.3.6/23;
set_real_ip_from 1.2.3.7/24;
more_clear_headers "X-Custom-Header";
add_header X-Frame-Options "DENY" always;
add_header Strict-Transport-Security "max-age=31536000; includeSubDomains" always;
add_header X-XSS-Protection "1; mode=block" always;
add_header X-Content-Type-Options "nosniff" always;
add_header Referrer-Policy "strict-origin-when-cross-origin" always;`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := serverSnippetClearHeaders(tt.snippet)
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("serverSnippetClearHeaders() = %v, want %v", got, tt.want)
			}
		})
	}
}
