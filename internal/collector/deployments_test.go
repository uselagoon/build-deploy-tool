...
func TestCollector_CollectDeployments(t *testing.T) {
	type args struct {
		ctx       context.Context
		namespace string
	}
	tests := []struct {
	...
	}{
		{
			...
			want:    "testdata/result/result-empty/lagoon-deployments.yaml",
			wantErr: false,
		},
		{
			...
			want:    "testdata/result/result-1/lagoon-deployments.yaml",
			wantErr: false,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := c.CollectDeployments(tt.args.ctx, tt.args.namespace)
			if (err != nil) != tt.wantErr {
				t.Errorf("Collector.CollectDeployments() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
