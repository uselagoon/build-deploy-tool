...
func TestCollector_CollectSchedulesV1Alpha1(t *testing.T) {
	type args struct {
		ctx       context.Context
		namespace string
	}
	tests := []struct {
	...
	}{
		{
			...
			want:    "testdata/result/result-empty/lagoon-schedules-v1alpha1.yaml",
			wantErr: false,
		},
		{
			name: "list-schedules",
			...
			want:    "testdata/result/result-1/lagoon-schedules-v1alpha1.yaml",
			wantErr: false,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := c.CollectSchedulesV1Alpha1(tt.args.ctx, tt.args.namespace)
			if (err != nil) != tt.wantErr {
				t.Errorf("Collector.CollectSchedulesV1Alpha1() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			...
			if string(oJ) != string(results) {
				t.Errorf("Collector.CollectSchedulesV1Alpha1() = \n%v", diff.LineDiff(string(results), string(oJ)))
			}
		})
	}
}
