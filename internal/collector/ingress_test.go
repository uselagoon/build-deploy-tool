...
func TestCollector_CollectIngress(t *testing.T) {
	type args struct {
		ctx       context.Context
		namespace string
	}
	tests := []struct {
	...
	}{
		{
			...
			want:    "testdata/result/result-empty/lagoon-ingress.yaml",
			wantErr: false,
		},
		{
			name: "list-services",
			...
			want:    "testdata/result/result-1/lagoon-ingress.yaml",
			wantErr: false,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := c.CollectIngress(tt.args.ctx, tt.args.namespace)
			if (err != nil) != tt.wantErr {
				t.Errorf("Collector.CollectIngress() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
