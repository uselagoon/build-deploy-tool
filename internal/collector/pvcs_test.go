...
func TestCollector_CollectPVCs(t *testing.T) {
	type args struct {
		ctx       context.Context
		namespace string
	}
	tests := []struct {
	...
	}{
		{
			...
			want:    "testdata/result/result-empty/lagoon-pvcs.yaml",
			wantErr: false,
		},
		{
			name: "list-secrets",
			...
			want:    "testdata/result/result-1/lagoon-pvcs.yaml",
			wantErr: false,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := c.CollectPVCs(tt.args.ctx, tt.args.namespace)
			if (err != nil) != tt.wantErr {
				t.Errorf("Collector.CollectPVCs() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
