...
func TestCollector_CollectCronjobs(t *testing.T) {
	type args struct {
		ctx       context.Context
		namespace string
	}
	tests := []struct {
	...
	}{
		{
			...
			want:    "testdata/result/result-empty/lagoon-cronjobs.yaml",
			wantErr: false,
		},
		{
			name: "list-cronjobs",
			...
			want:    "testdata/result/result-1/lagoon-cronjobs.yaml",
			wantErr: false,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := c.CollectCronjobs(tt.args.ctx, tt.args.namespace)
			if (err != nil) != tt.wantErr {
				t.Errorf("Collector.CollectCronjobs() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
