...
func TestCollector_CollectSecrets(t *testing.T) {
	type args struct {
		ctx       context.Context
		namespace string
	}
	tests := []struct {
		...
		},
		{
			...
			want:    "testdata/result/result-1/lagoon-secrets.yaml",
			wantErr: false,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := c.CollectSecrets(tt.args.ctx, tt.args.namespace)
			if (err != nil) != tt.wantErr {
				t.Errorf("Collector.CollectSecrets() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
