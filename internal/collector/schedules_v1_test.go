...
func TestCollector_CollectSchedulesV1(t *testing.T) {
	type args struct {
		ctx       context.Context
		namespace string
	}
	tests := []struct {
	...
	}{
		{
			...
			want:    "testdata/result/result-empty/lagoon-schedules-v1.yaml",
			wantErr: false,
		},
		{
			name: "list-schedules",
			...
			want:    "testdata/result/result-1/lagoon-schedules-v1.yaml",
			wantErr: false,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := c.CollectSchedulesV1(tt.args.ctx, tt.args.namespace)
			if (err != nil) != tt.wantErr {
				t.Errorf("Collector.CollectSchedulesV1() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
