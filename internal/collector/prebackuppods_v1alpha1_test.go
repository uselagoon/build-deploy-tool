...
func TestCollector_CollectPreBackupPodsV1Alpha1(t *testing.T) {
	type args struct {
		ctx       context.Context
		namespace string
	}
	tests := []struct {
	...
	}{
		{
			...
			want:    "testdata/result/result-empty/lagoon-prebackuppods-v1alpha1.yaml",
			wantErr: false,
		},
		{
			name: "list-prebackuppods",
			args: args{
				ctx:       context.Background(),
				namespace: "example-project-main",
			},
			seedDir: "testdata/seed/seed-1",
			want:    "testdata/result/result-1/lagoon-prebackuppods-v1alpha1.yaml",
			wantErr: false,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := c.CollectPreBackupPodsV1Alpha1(tt.args.ctx, tt.args.namespace)
			if (err != nil) != tt.wantErr {
				t.Errorf("Collector.CollectPreBackupPodsV1Alpha1() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			...
			if string(oJ) != string(results) {
				t.Errorf("Collector.CollectPreBackupPodsV1Alpha1() = \n%v", diff.LineDiff(string(results), string(oJ)))
			}
		})
	}
}
