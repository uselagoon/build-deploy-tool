...
func TestCollector_CollectPostgreSQLConsumers(t *testing.T) {
	type args struct {
		ctx       context.Context
		namespace string
	}
	tests := []struct {
	...
	}{
		{
			...
			want:    "testdata/result/result-empty/lagoon-postgres-consumers.yaml",
			wantErr: false,
		},
		{
			name: "list-postgres-consumers",
			args: args{
				ctx:       context.Background(),
				namespace: "example-project-main",
			},
			seedDir: "testdata/seed/seed-1",
			want:    "testdata/result/result-1/lagoon-postgres-consumers.yaml",
			wantErr: false,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := c.CollectPostgreSQLConsumers(tt.args.ctx, tt.args.namespace)
			if (err != nil) != tt.wantErr {
				t.Errorf("Collector.CollectPostgreSQLConsumers() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			...
			if string(oJ) != string(results) {
				t.Errorf("Collector.CollectPostgreSQLConsumers() = \n%v", diff.LineDiff(string(results), string(oJ)))
			}
		})
	}
}
