...
// MergeApexAlternativeNames looks through the provided routes for pairs where an apex
// domain (e.g. `example.com`) and its `www.` counterpart (e.g. `www.example.com`) have
// both been defined as separate routes. Where a pair is found, and merging them won't
// change the resulting behaviour, one is folded into the other as an `AlternativeNames`
// entry and removed from the returned list - this results in a single ingress object
// being generated for both domains, instead of one ingress object each.
//
// Whichever of the two domains was defined first (i.e. appears earliest in the provided
// slice) is the one that is retained as the ingress's primary host and keeps its original
// position in the list; the other is folded in as an alternativeName. This preserves the
// user's original ordering/intent - e.g. if `www.example.com` is listed before
// `example.com`, the merged ingress keeps `www.example.com` as its host, and
// `example.com` becomes the alternativeName, not the other way around.
//
// A pair is only merged when it is unambiguously safe to do so:
//   - both routes must target the same lagoon service
//   - neither route can be autogenerated or a wildcard route
//   - the route being folded away can't carry its own alternative names or path routes,
//     as that configuration would otherwise be silently dropped
//   - every setting that affects the generated ingress object (tls-acme, insecure,
//     ingress class, monitoring path, fastly, request verification, migrate, hsts,
//     annotations and labels) must match exactly between the two routes
//
// If any of the above don't line up, both routes are left completely untouched, so a
// merge attempt can never change or lose a user's configuration - it can only ever
// reduce the number of ingress objects created.
func MergeApexAlternativeNames(routes []RouteV2) []RouteV2 {
	byDomain := map[string]int{}
	for i, r := range routes {
		byDomain[r.Domain] = i
	}

	folded := map[string]bool{}   // domains that have been merged away and should be dropped
	resolved := map[string]bool{} // domains that have already been considered as part of a pair
	...
	for i := range routes {
		...
		var apexDomain, wwwDomain string
		if strings.HasPrefix(domain, "www.") {
			wwwDomain = domain
			apexDomain = strings.TrimPrefix(domain, "www.")
		} else {
			apexDomain = domain
			wwwDomain = "www." + domain
		}

		apexIdx, apexOk := byDomain[apexDomain]
		wwwIdx, wwwOk := byDomain[wwwDomain]
		if !apexOk || !wwwOk {
			// no counterpart defined, nothing to do for this domain
			continue
		}

		resolved[apexDomain] = true
		resolved[wwwDomain] = true

		if !canMergeWWWRoute(routes[apexIdx], routes[wwwIdx]) {
			continue
		}

		// whichever domain appears earliest in the original list is retained as-is;
		// the later one is folded into it as an alternativeName
		keepIdx, dropIdx := apexIdx, wwwIdx
		if wwwIdx < apexIdx {
			keepIdx, dropIdx = wwwIdx, apexIdx
		}

		routes[keepIdx].AlternativeNames = helpers.AppendIfMissing(routes[keepIdx].AlternativeNames, routes[dropIdx].Domain)
		folded[routes[dropIdx].Domain] = true
	}

	if len(folded) == 0 {
		return routes
	}
	final := make([]RouteV2, 0, len(routes))
...
// canMergeWWWRoute checks that folding `www` into `apex` won't change either route's
// resulting ingress behaviour in any way.
func canMergeWWWRoute(apex, www RouteV2) bool {
	if apex.Autogenerated || www.Autogenerated {
		return false
	}
	if boolPtrTrue(apex.Wildcard) || boolPtrTrue(www.Wildcard) {
		return false
	}
	if apex.LagoonService != www.LagoonService {
		return false
	}
	// don't swallow a `www` route that has its own alternative names or path routes,
	// that configuration would otherwise be lost
	if len(www.AlternativeNames) > 0 || len(www.PathRoutes) > 0 {
		return false
	}
	if !boolPtrEqual(apex.TLSAcme, www.TLSAcme) {
		return false
	}
	if !strPtrEqual(apex.Insecure, www.Insecure) {
		return false
	}
	if apex.IngressClass != www.IngressClass {
		return false
	}
	if apex.MonitoringPath != www.MonitoringPath {
		return false
	}
	if !boolPtrEqual(apex.Migrate, www.Migrate) {
		return false
	}
	if !boolPtrEqual(apex.RequestVerification, www.RequestVerification) {
		return false
	}
	if !boolPtrEqual(apex.HSTSEnabled, www.HSTSEnabled) {
		return false
	}
	if apex.HSTSMaxAge != www.HSTSMaxAge {
		return false
	}
	if !boolPtrEqual(apex.HSTSIncludeSubdomains, www.HSTSIncludeSubdomains) {
		return false
	}
	if !boolPtrEqual(apex.HSTSPreload, www.HSTSPreload) {
		return false
	}
	if !reflect.DeepEqual(apex.Fastly, www.Fastly) {
		return false
	}
	if !reflect.DeepEqual(apex.Annotations, www.Annotations) {
		return false
	}
	if !reflect.DeepEqual(apex.Labels, www.Labels) {
		return false
	}
	return true
}
