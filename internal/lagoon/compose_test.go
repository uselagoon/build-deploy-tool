...
func TestUnmarshaDockerComposeYAML(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args: args{
				file: "internal/testdata/docker-compose/test2/lagoon.yml",
			},
			want: `{"name":"test2","services":{"node":{"build":{"context":".","dockerfile":"node.dockerfile"},"command":null,"entrypoint":null,"environment":{"LAGOON_LOCALDEV_HTTP_PORT":"3000","LAGOON_ROUTE":"http://node.docker.amazee.io"},"labels":{"lagoon.type":"node"},"networks":{"amazeeio-network":null,"default":null},"volumes":[{"type":"bind","source":".","target":"/app","bind":{"create_host_path":true}}]}},"networks":{"amazeeio-network":{"name":"amazeeio-network","ipam":{},"external":true},"default":{"name":"test2_default","ipam":{}}}}`,
			wantServiceOrder: []OriginalServiceOrder{
				{Index: 0, Name: "node"},
			},
			wantVolumeOrder: []OriginalVolumeOrder{},
		...
		},
		{
			...
			args: args{
				file: "internal/testdata/docker-compose/test11/lagoon.yml",
			},
			wantErr:    true,
		...
		},
		{
			...
			args: args{
				file: "internal/testdata/docker-compose/test12/lagoon.yml",
			},
			want: `{"name":"test12","services":{"node":{"build":{"context":".","dockerfile":"node.dockerfile"},"command":null,"entrypoint":null,"environment":{"LAGOON_LOCALDEV_HTTP_PORT":"3000","LAGOON_ROUTE":"http://node.docker.amazee.io"},"labels":{"lagoon.type":"node"},"networks":{"amazeeio-network":null,"default":null},"volumes":[{"type":"volume","source":"node","target":"/app","volume":{}},{"type":"volume","source":"config","target":"/config","volume":{}},{"type":"volume","source":"files","target":"/files","volume":{}}]}},"networks":{"amazeeio-network":{"name":"amazeeio-network","ipam":{},"external":true},"default":{"name":"test12_default","ipam":{}}},"volumes":{"config":{"name":"test12_config","labels":{"lagoon.type":"persistent"}},"db":{"name":"test12_db","labels":{"lagoon.type":"none"}},"files":{"name":"test12_files","labels":{"lagoon.persistent.size":"10Gi","lagoon.type":"persistent"}},"logs":{"name":"test12_logs"},"node":{"name":"test12_node","labels":{"lagoon.type":"persistent"}},"notused":{"name":"test12_notused","labels":{"lagoon.type":"persistent"}}}}`,
			wantServiceOrder: []OriginalServiceOrder{
				{Index: 0, Name: "node"},
			},
			wantVolumeOrder: []OriginalVolumeOrder{
		...
		{
			name: "test13 check a valid docker-compose with newer version of optional env_files",
			args: args{
				file:                     "internal/testdata/docker-compose/test13/lagoon.yml",
				ignoreNonStringKeyErrors: true,
				ignoreMissingEnvFiles:    true,
			},
			want: `{"name":"test13","services":{"cli":{"build":{"context":".","dockerfile":"lagoon/cli.dockerfile"},"command":null,"container_name":"test13-cli","entrypoint":null,"environment":{"DRUSH_OPTIONS_URI":"https://","LAGOON_PROJECT":"test13","LAGOON_ROUTE":"https://","SIMPLETEST_BASE_URL":"http://nginx:8080","SIMPLETEST_DB":"mysql://drupal:drupal@mariadb:3306/drupal","SSMTP_MAILHUB":"host.docker.internal:1025"},"image":"test13","labels":{"lagoon.persistent":"/app/public/sites/default/files/","lagoon.persistent.name":"nginx","lagoon.type":"cli-persistent"},"networks":{"default":null},"volumes":[{"type":"bind","source":".","target":"/app","bind":{"create_host_path":true}},{"type":"volume","source":"ssh","target":"/tmp/amazeeio_ssh-agent","volume":{}}]},"mariadb":{"command":null,"container_name":"test13-db","entrypoint":null,"environment":{"LAGOON_PROJECT":"test13","LAGOON_ROUTE":"https://","SSMTP_MAILHUB":"host.docker.internal:1025"},"image":"uselagoon/mariadb-drupal:latest","labels":{"lagoon.type":"mariadb"},"networks":{"default":null},"ports":[{"mode":"ingress","target":3306,"protocol":"tcp"}]},"nginx":{"build":{"context":".","dockerfile":"lagoon/nginx.dockerfile","args":{"CLI_IMAGE":"test13"}},"command":null,"container_name":"test13-nginx","depends_on":{"cli":{"condition":"service_started","required":true}},"entrypoint":null,"environment":{"LAGOON_LOCALDEV_URL":"","LAGOON_PROJECT":"test13","LAGOON_ROUTE":"https://","SSMTP_MAILHUB":"host.docker.internal:1025"},"labels":{"lagoon.persistent":"/app/public/sites/default/files/","lagoon.type":"nginx-php-persistent"},"networks":{"default":null,"stonehenge-network":null},"volumes":[{"type":"bind","source":".","target":"/app","bind":{"create_host_path":true}}]},"php":{"build":{"context":".","dockerfile":"lagoon/php.dockerfile","args":{"CLI_IMAGE":"test13"}},"command":null,"container_name":"test13-php","depends_on":{"cli":{"condition":"service_started","required":true}},"entrypoint":null,"environment":{"LAGOON_PROJECT":"test13","LAGOON_ROUTE":"https://","SSMTP_MAILHUB":"host.docker.internal:1025"},"labels":{"lagoon.name":"nginx","lagoon.persistent":"/app/public/sites/default/files/","lagoon.type":"nginx-php-persistent"},"networks":{"default":null},"volumes":[{"type":"bind","source":".","target":"/app","bind":{"create_host_path":true}}]},"pma":{"command":null,"container_name":"test13-pma","entrypoint":null,"environment":{"PMA_HOST":"mariadb","PMA_PASSWORD":"drupal","PMA_USER":"drupal","UPLOAD_LIMIT":"1G"},"image":"phpmyadmin/phpmyadmin","labels":{"lagoon.type":"none"},"networks":{"default":null,"stonehenge-network":null}}},"networks":{"default":{"name":"test13_default","ipam":{}},"stonehenge-network":{"name":"stonehenge-network","ipam":{},"external":true}},"volumes":{"es_data":{"name":"test13_es_data"},"ssh":{"name":"stonehenge-ssh","external":true}}}`,
			wantServiceOrder: []OriginalServiceOrder{
				{Index: 0, Name: "cli"},
				{Index: 1, Name: "nginx"},
				{Index: 2, Name: "php"},
		...
		{
			name: "test14 check a valid docker-compose with newer version of optional env_files (same as test12 but not ignoring the errors)",
			args: args{
				file: "internal/testdata/docker-compose/test14/lagoon.yml",
			},
			wantErr:    true,
			wantErrMsg: "no such file or directory",
		},
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr {
				...
				if !cmp.Equal(string(stra), tt.want) {
					t.Errorf("UnmarshaDockerComposeYAML() = %v\n, want\n %v", string(stra), tt.want)
				}
			}
			if !cmp.Equal(dcpo, tt.wantServiceOrder) {
				t.Errorf("UnmarshaDockerComposeYAML() = %v\n, want\n %v", dcpo, tt.wantServiceOrder)
			}
			if !cmp.Equal(dcvo, tt.wantVolumeOrder) {
				t.Errorf("UnmarshaDockerComposeYAML() = %v\n, want\n %v", dcvo, tt.wantVolumeOrder)
			}
		})
	}
}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := CheckDockerComposeLagoonLabel(tt.args.labels, tt.args.label); got != tt.want {
				t.Errorf("CheckDockerComposeLagoonLabel() = %v\n, want\n %v", got, tt.want)
			}
		})
	}
}
