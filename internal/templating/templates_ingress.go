...
func GenerateIngressTemplate(
	...
	if lValues.EnvironmentType == "development" || route.Autogenerated {
		...
		if route.IngressClass == "traefik" {
			additionalAnnotations["traefik.ingress.kubernetes.io/router.middlewares"] = addMiddleware(
				additionalAnnotations["traefik.ingress.kubernetes.io/router.middlewares"], fmt.Sprintf("%s-x-robots@kubernetescrd", lValues.Namespace),
			)
		}
	}

	switch route.Source {
	case "API", "AUTOGENERATED":
		additionalLabels["route.lagoon.sh/source"] = strings.ToLower(route.Source)
	default:
		additionalLabels["route.lagoon.sh/source"] = "yaml"
	}
	...
	for key := range route.Annotations {
		if route.IngressClass == "traefik" && key == "nginx.ingress.kubernetes.io/permanent-redirect" {
			additionalAnnotations["traefik.ingress.kubernetes.io/router.middlewares"] = addMiddleware(
