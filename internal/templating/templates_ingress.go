...
func GenerateIngressTemplate(
	...
	ingress.Name = route.IngressName
	...
	if err := apivalidation.ValidateAnnotations(ingress.Annotations, nil); err != nil {
		if len(err) != 0 {
			return nil, fmt.Errorf("the annotations for %s are not valid: %v", route.Domain, err)
		}
	}
	// validate any labels
	if err := metavalidation.ValidateLabels(ingress.Labels, nil); err != nil {
		if len(err) != 0 {
