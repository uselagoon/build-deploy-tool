...
func GenerateIngressTemplate(
	...
	if lValues.EnvironmentType == "production" {
		...
		if lValues.Monitoring.Enabled && !route.Autogenerated && primaryIngress.Host == route.Domain {
			...
			// only add the monitoring annotations if monitoring is enabled
	...
	if lValues.Oauth2ProxyDomain != "" && *route.OauthProtected {
		...
		if route.Autogenerated {
			...
			additionalAnnotations["nginx.ingress.kubernetes.io/auth-url"] = lValues.Oauth2ProxyDomain + "/oauth2/auth?namespace=" + lValues.Namespace 
		} else {
			additionalAnnotations["nginx.ingress.kubernetes.io/auth-signin"] = "http://" + route.Oauth2ProxyDomain + "/oauth2/start?rd=$scheme://$host$request_uri"
			additionalAnnotations["nginx.ingress.kubernetes.io/auth-url"] = "http://" + route.Oauth2ProxyDomain + "/oauth2/auth?namespace=" + lValues.Namespace 
		}
	}
