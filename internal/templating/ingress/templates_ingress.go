...
func GenerateIngressTemplate(
	...
	ingress.ObjectMeta.Name = route.Domain
	...
	if route.Wildcard != nil && *route.Wildcard == true {
		// prefix the object name with wildcard
		ingress.ObjectMeta.Name = fmt.Sprintf("wildcard-%s", ingress.ObjectMeta.Name)
		// if the new name exceeds the validation spec, truncate it
		if err := validation.IsDNS1123Subdomain(strings.ToLower(ingress.ObjectMeta.Name)); err != nil {
			ingress.ObjectMeta.Name = fmt.Sprintf("%s-%s", ingress.ObjectMeta.Name[:len(ingress.ObjectMeta.Name)-10], helpers.GetMD5HashWithNewLine(route.Domain)[:5])
		}
