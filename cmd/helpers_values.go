...
func collectBuildValues(debug bool, activeEnv, standbyEnv *bool,
	...
	lagoonValues.Route, lagoonValues.Routes, err = generateRoutes(
		mergedVariables,
		*lagoonValues,
		*lYAML,
		autogenRoutes,
...
func collectBuildVariables(lagoonValues lagoon.BuildValues) []lagoon.EnvironmentVariable {
	...
	vars = append(vars, lagoon.EnvironmentVariable{Name: "LAGOON_ROUTE", Value: lagoonValues.Route, Scope: "runtime"})                      //deprecated???
	vars = append(vars, lagoon.EnvironmentVariable{Name: "LAGOON_ROUTES", Value: strings.Join(lagoonValues.Routes, ","), Scope: "runtime"}) //deprecated???
	return vars
}
